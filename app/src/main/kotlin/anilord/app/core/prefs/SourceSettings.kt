...
import anilord.app.settings.utils.validation.DomainValidator
import java.io.File
...
class SourceSettings(context: Context, source: MangaSource) : MangaSourceConfig {
	...
	override fun <T> get(key: ConfigKey<T>): T {
		return when (key) {
			...
			is ConfigKey.Username -> prefs.getString(key.key, key.defaultValue)
				.ifNullOrEmpty { key.defaultValue }
			is ConfigKey.Password -> prefs.getString(key.key, key.defaultValue)
				.ifNullOrEmpty { key.defaultValue }
		} as T
	}
	...
	operator fun <T> set(key: ConfigKey<T>, value: T) = prefs.edit {
		when (key) {
			...
			is ConfigKey.Username -> putString(key.key, value as String?)
			is ConfigKey.Password -> putString(key.key, value as String?)
		}
	}
