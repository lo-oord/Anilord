...
class CommonHeadersInterceptor @Inject constructor(
	...
	override fun intercept(chain: Chain): Response {
		...
		val newRequest = request.newBuilder()
			...
			.apply {
				// Coil conveys the source through an internal header. Preserve it as a typed
				// request tag before removing that header so response interceptors (notably
				// Cloudflare detection) still know which source was challenged.
				if (source != null) {
