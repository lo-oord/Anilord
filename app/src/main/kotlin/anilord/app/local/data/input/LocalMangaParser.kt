package anilord.app.local.data.input
...
import anilord.app.core.util.ext.isDirectory
import anilord.app.core.util.ext.isFileUri
import anilord.app.core.util.ext.isImage
import anilord.app.core.util.ext.isRegularFile
import anilord.app.core.util.ext.isZipUri
import anilord.app.core.util.ext.printStackTraceDebug
import anilord.app.core.util.ext.toFileNameSafe
import anilord.app.core.util.ext.toListSorted
import anilord.app.local.data.MangaIndex
import anilord.app.local.data.hasZipExtension
import anilord.app.local.data.isZipArchive
import anilord.app.local.data.output.LocalMangaOutput.Companion.ENTRY_NAME_INDEX
...
class LocalMangaParser(private val uri: Uri) {
	...
	suspend fun getPages(chapter: MangaChapter): List<MangaPage> = runInterruptible(Dispatchers.IO) {
		...
		chapterUri.resolveFsAndPath().use { (fileSystem, rootPath) ->
			...
			if (index != null) {
				...
				// A downloaded novel chapter is stored as an HTML file inside its CBZ.
				// CBZ files for individual chapters do not contain an index, so filtering
				// images only made an offline novel chapter appear empty.
				entries.filter { x -> (x.isImage() || x.isNovelContent()) && x.parent == rootPath }
	...
	companion object {
		...
		private fun Path.isNovelContent(): Boolean =
