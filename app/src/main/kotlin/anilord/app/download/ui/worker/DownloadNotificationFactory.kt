...
import anilord.app.core.ErrorReporterReceiver
...
import anilord.app.core.util.ext.getDrawableOrThrow
import anilord.app.core.util.ext.getNotificationIconSize
import anilord.app.core.util.ext.isReportable
...
class DownloadNotificationFactory @AssistedInject constructor(
	...
	init {
		...
		builder.priority = if (isSilent) NotificationCompat.PRIORITY_LOW else NotificationCompat.PRIORITY_DEFAULT
	}
	...
	suspend fun create(state: DownloadState?): Notification = mutex.withLock {
		...
		when {
			...
			state.error != null -> { // error, final state
				...
				builder.setProgress(state.max, state.progress, state.isIndeterminate)
				builder.setContentText(
					if (state.isIndeterminate) null else getProgressString(state.percent, state.eta, state.isStuck),
				)
				builder.setCategory(NotificationCompat.CATEGORY_PROGRESS)
				builder.setStyle(null)
				builder.setOngoing(true)
	...
	private fun createChannels() {
		...
		manager.createNotificationChannel(
			// Foreground-service notifications must be LOW or higher. Some Android builds
			// silently reject an IMPORTANCE_MIN notification, leaving WorkManager's
			// SystemForegroundService to hit the five-second startForeground timeout.
			NotificationChannelCompat.Builder(CHANNEL_ID_SILENT, NotificationManagerCompat.IMPORTANCE_LOW)
