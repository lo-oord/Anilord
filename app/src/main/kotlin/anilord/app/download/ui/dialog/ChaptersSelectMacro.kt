...
interface ChaptersSelectMacro {
	...
	class NotDownloadedChapters(
		val chaptersCount: Int,
		val maxAvailableCount: Int,
		private val downloadedChapterIds: Map<Long, Set<Long>>,
		private val preferredBranches: Map<Long, String?>,
	) : ChaptersSelectMacro {

		override fun getChaptersIds(mangaId: Long, chapters: List<MangaChapter>): Set<Long> {
			val downloaded = downloadedChapterIds[mangaId].orEmpty()
			if (chapters.isEmpty() || downloaded.isEmpty()) {
				return emptySet()
			}
			val preferredBranch = preferredBranches[mangaId]
			var branch = preferredBranch
			var lastDownloadedIndex = chapters.indexOfLast { chapter ->
				chapter.branch == preferredBranch && chapter.id in downloaded
			}
			if (lastDownloadedIndex < 0) {
				lastDownloadedIndex = chapters.indexOfLast { it.id in downloaded }
				.takeIf { it >= 0 } ?: return emptySet()
				branch = chapters[lastDownloadedIndex].branch
			}
			val result = ArraySet<Long>(minOf(chaptersCount, chapters.size))
			for (index in lastDownloadedIndex + 1 until chapters.size) {
				val chapter = chapters[index]
				if (chapter.branch == branch && chapter.id !in downloaded) {
		...
		fun copy(count: Int) = NotDownloadedChapters(
			count,
			maxAvailableCount,
