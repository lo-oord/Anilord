...
class ProgressUpdateUseCase @Inject constructor(
	...
	suspend operator fun invoke(manga: Manga): Float {
		...
		val currentPosition = ppc * chapterIndex + ppc * pagePercent
		val savedFrontier = if (ReadingProgress.isValid(history.percent) && history.chaptersCount > 0) {
			(history.percent * history.chaptersCount / chaptersCount).coerceIn(0f, 1f)
		} else {
			PROGRESS_NONE
		}
		val result = maxOf(currentPosition, savedFrontier)
