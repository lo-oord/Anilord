...
import anilord.app.core.db.entity.toEntity
import anilord.app.core.db.entity.toManga
import anilord.app.core.db.entity.toMangaList
import anilord.app.core.db.entity.toMangaTags
import anilord.app.core.db.entity.toMangaTagsList
...
import anilord.app.core.model.toMangaSources
import anilord.app.core.parser.MangaDataRepository
import anilord.app.core.prefs.AppSettings
import anilord.app.core.prefs.ProgressIndicatorMode
...
import anilord.app.scrobbling.common.domain.tryScrobble
import anilord.app.search.domain.SearchKind
import anilord.app.tracker.domain.CheckNewChaptersUseCase
...
class HistoryRepository @Inject constructor(
	...
	suspend fun addOrUpdate(manga: Manga, chapterId: Long, page: Int, scroll: Int, percent: Float, force: Boolean) {
		...
		db.withTransaction {
			...
			val previous = if (force) null else db.getHistoryDao().find(manga.id)
			val savedPercent = previous?.let {
				rescaleProgress(it.percent, it.chaptersCount, chaptersCount)
			...
			db.getHistoryDao().upsert(
				HistoryEntity(
					...
					// Opening an older chapter changes the resume position, but must not erase
					// the furthest chapter the user has already read.
					percent = maxOf(percent, savedPercent),
	...
	private fun rescaleProgress(percent: Float, oldCount: Int, newCount: Int): Float {
		if (!ReadingProgress.isValid(percent) || oldCount <= 0 || newCount <= 0) {
			return ReadingProgress.PROGRESS_NONE
		}
		return (percent * oldCount / newCount).coerceIn(0f, 1f)
	}
