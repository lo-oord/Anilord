...
import org.manga.peak.settings.utils.AutoCompleteTextViewPreference
import org.manga.peak.settings.utils.EditTextBindListener
import org.manga.peak.settings.utils.EditTextDefaultSummaryProvider
import org.manga.peak.settings.utils.validation.DomainValidator
import org.manga.peak.settings.utils.validation.HeaderValidator
		...
		val preference: Preference = when (key) {
			...
			is ConfigKey.Username -> {
				EditTextPreference(screen.context).apply {
					...
					setDialogTitle(R.string.username)
				}
			}

			is ConfigKey.Password -> {
				EditTextPreference(screen.context).apply {
					summaryProvider = EditTextDefaultSummaryProvider(key.defaultValue)
					setOnBindEditTextListener(
						EditTextBindListener(
