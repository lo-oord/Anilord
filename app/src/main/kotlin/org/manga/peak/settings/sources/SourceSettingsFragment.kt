package org.manga.peak.settings.sources
...
import org.manga.peak.core.parser.EmptyMangaRepository
...
class SourceSettingsFragment : BasePreferenceFragment(0), Preference.OnPreferenceChangeListener {
	...
	override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
		...
		findPreference<Preference>(KEY_AUTH)?.run {
			...
			if (authProvider is MangaParserCredentialsAuthProvider) {
				setTitle(R.string.anime_witcher_account)
			}
		}
	...
	override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
		...
		val credentialsAuth = (viewModel.repository as? ParserMangaRepository)
			?.getAuthProvider() is MangaParserCredentialsAuthProvider
		viewModel.isAuthorized.filterNotNull().observe(viewLifecycleOwner) { isAuthorized ->
			findPreference<Preference>(KEY_AUTH)?.isEnabled = credentialsAuth || !isAuthorized
		}
