...
import org.manga.peak.core.util.ext.toLocaleOrNull
...
fun MangaSource.getLocaleCode(): String? = when (val source = unwrap()) {
	is MangaParserSource -> source.locale
	else -> null
}

fun MangaSource.getLocale(): Locale? = getLocaleCode()?.toLocaleOrNull()
