...
import org.manga.peak.alternatives.ui.AlternativesActivity
import org.manga.peak.backups.ui.backup.BackupDialogFragment
import org.manga.peak.backups.ui.restore.RestoreDialogFragment
import org.manga.peak.bookmarks.ui.AllBookmarksActivity
import org.manga.peak.browser.BrowserActivity
import org.manga.peak.browser.cloudflare.CloudFlareActivity
...
import org.manga.peak.core.model.appUrl
import org.manga.peak.core.model.getTitle
import org.manga.peak.core.model.isAnime
import org.manga.peak.core.model.isBroken
import org.manga.peak.core.model.isLocal
import org.manga.peak.core.model.parcelable.ParcelableManga
import org.manga.peak.core.model.parcelable.ParcelableMangaListFilter
import org.manga.peak.core.model.parcelable.ParcelableMangaPage
import org.manga.peak.core.network.CommonHeaders
import org.manga.peak.core.parser.external.ExternalMangaSource
...
import org.manga.peak.core.ui.dialog.BigButtonsAlertDialog
import org.manga.peak.core.ui.dialog.ErrorDetailsDialog
...
import org.manga.peak.core.util.ext.getThemeDrawable
import org.manga.peak.core.util.ext.printStackTraceDebug
import org.manga.peak.core.util.ext.toFileOrNull
import org.manga.peak.core.util.ext.toUriOrNull
import org.manga.peak.core.util.ext.withArgs
import org.manga.peak.details.ui.DetailsActivity
import org.manga.peak.details.ui.pager.ChaptersPagesSheet
import org.manga.peak.details.ui.related.RelatedMangaActivity
import org.manga.peak.details.ui.scrobbling.ScrobblingInfoSheet
import org.manga.peak.download.ui.dialog.DownloadDialogFragment
import org.manga.peak.download.ui.list.DownloadsActivity
import org.manga.peak.favourites.ui.FavouritesActivity
import org.manga.peak.favourites.ui.categories.FavouriteCategoriesActivity
import org.manga.peak.favourites.ui.categories.edit.FavouritesCategoryEditActivity
import org.manga.peak.favourites.ui.categories.select.FavoriteDialog
import org.manga.peak.filter.ui.FilterCoordinator
import org.manga.peak.filter.ui.sheet.FilterSheetFragment
import org.manga.peak.filter.ui.tags.TagsCatalogSheet
import org.manga.peak.history.ui.HistoryActivity
import org.manga.peak.image.ui.ImageActivity
import org.manga.peak.list.ui.config.ListConfigBottomSheet
import org.manga.peak.list.ui.config.ListConfigSection
import org.manga.peak.local.ui.ImportDialogFragment
import org.manga.peak.local.ui.info.LocalInfoDialog
import org.manga.peak.main.ui.MainActivity
import org.manga.peak.main.ui.welcome.WelcomeSheet
...
import org.manga.peak.reader.ui.colorfilter.ColorFilterConfigActivity
import org.manga.peak.reader.ui.config.ReaderConfigSheet
import org.manga.peak.reader.ui.novel.NovelReaderConfigSheet
import org.manga.peak.scrobbling.common.domain.model.ScrobblerService
import org.manga.peak.scrobbling.common.ui.config.ScrobblerConfigActivity
import org.manga.peak.scrobbling.common.ui.selector.ScrobblingSelectorSheet
import org.manga.peak.search.domain.SearchKind
import org.manga.peak.search.ui.MangaListActivity
import org.manga.peak.search.ui.multi.SearchActivity
import org.manga.peak.settings.SettingsActivity
import org.manga.peak.settings.override.OverrideConfigActivity
import org.manga.peak.settings.reader.ReaderTapGridConfigActivity
import org.manga.peak.settings.sources.auth.CredentialsSourceAuthActivity
import org.manga.peak.settings.sources.auth.SourceAuthActivity
import org.manga.peak.settings.sources.catalog.SourcesCatalogActivity
import org.manga.peak.settings.storage.MangaDirectorySelectDialog
import org.manga.peak.settings.storage.directories.MangaDirectoriesActivity
import org.manga.peak.settings.tracker.categories.TrackerCategoriesConfigSheet
import org.manga.peak.stats.ui.StatsActivity
import org.manga.peak.stats.ui.sheet.MangaStatsSheet
import org.manga.peak.suggestions.ui.SuggestionsActivity
import org.manga.peak.tracker.ui.updates.UpdatesActivity
import java.io.File
...
class AppRouter private constructor(
    ...
    @CheckResult
    fun openPlayStore(chooserTitle: CharSequence? = null): Boolean {
        val marketIntent = Intent(Intent.ACTION_VIEW, "market://details?id=org.manga.peak".toUri())
            .setPackage("com.android.vending")
        return startActivitySafe(marketIntent) || openExternalBrowser(
            url = contextOrNull()?.getString(R.string.url_google_play) ?: return false,
	...
	fun showDownloadDialog(manga: Collection<Manga>, snackbarHost: View?) {
		val downloadableManga = manga.filterNot { it.isLocal }
        if (downloadableManga.isEmpty()) {
            return
        }
        ...
        DownloadDialogFragment().withArgs(1) {
            putParcelableArray(KEY_MANGA, downloadableManga.mapToArray { ParcelableManga(it, withDescription = false) })
        }.showDistinct()
    }
	...
	fun showNovelConfigSheet() {
		NovelReaderConfigSheet().showDistinct()
	}
    ...
    companion object {
        ...
        fun sourceAuthIntent(context: Context, source: MangaSource): Intent {
            val activityClass = if (source.name == MangaParserSource.ANIME_WITCHER.name) {
                CredentialsSourceAuthActivity::class.java
            } else {
                SourceAuthActivity::class.java
            }
        ...
        const val KEY_AUTH_RETURN_ON_SUCCESS = "auth_return_on_success"
