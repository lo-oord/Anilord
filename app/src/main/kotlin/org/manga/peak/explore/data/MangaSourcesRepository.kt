...
import org.manga.peak.core.model.MangaSource as resolveMangaSource
...
import org.manga.peak.core.ui.util.ReversibleHandle
import org.manga.peak.core.util.ext.flattenLatest
...
class MangaSourcesRepository @Inject constructor(
	...
	val allMangaSources: Set<MangaSource> = Collections.unmodifiableSet(
		buildSet {
			addAll(MangaParserSource.entries.filterNot(MangaParserSource::isBroken))
		},
	)
	...
	suspend fun getDisabledSources(): Set<MangaSource> {
		...
		val result = HashSet(allMangaSources)
	...
	suspend fun querySources(
		isDisabledOnly: Boolean,
		isNewOnly: Boolean,
		excludeBroken: Boolean,
		types: Set<SourceContentType>,
		query: String?,
		locale: String?,
		...
		val sources = entities.toSources(
			...
			sortOrder = sortOrder,
		).mapTo(ArrayList()) { it.mangaSource }
		if (locale != null) {
			sources.retainAll { it.getLocaleCode() == locale }
		}
		if (excludeBroken) {
			sources.removeAll { (it as? MangaParserSource)?.isBroken == true }
		}
		if (types.isNotEmpty()) {
			sources.retainAll { it.sourceContentType in types }
		}
	...
	private suspend fun assimilateNewSources(): Boolean {
		...
		initializeNovelSources()
		initializeAnimeSources()
		return new.isNotEmpty()
	}

	/** Makes the novel catalogue visible once for existing installations without overriding later user choices. */
	private suspend fun initializeNovelSources() {
		if (settings.areNovelSourcesInitialized) return
		val novelSources = allMangaSources.filter { it.sourceContentType == SourceContentType.NOVEL }
		if (novelSources.isEmpty()) return
		for (source in novelSources) {
			dao.setEnabled(source.name, true)
		}
		settings.areNovelSourcesInitialized = true
	}

	/** Makes anime sources visible once for existing installations without overriding later user choices. */
	private suspend fun initializeAnimeSources() {
		if (settings.areAnimeSourcesInitialized) return
		val animeSources = allMangaSources.filter { it.sourceContentType == SourceContentType.ANIME }
		if (animeSources.isEmpty()) return
		for (source in animeSources) {
			dao.setEnabled(source.name, true)
		}
		settings.areAnimeSourcesInitialized = true
	}
	...
	private fun String.toMangaSourceOrNull(): MangaSource? = resolveMangaSource(this).takeUnless {
		it === UnknownMangaSource
	}
}
