...
import org.manga.peak.core.ui.widgets.TwoLinesItemView
...
import org.manga.peak.core.util.ext.showOrHide
import org.manga.peak.databinding.DialogDownloadBinding
...
class DownloadDialogFragment : AlertDialogFragment<DialogDownloadBinding>(), View.OnClickListener {
	...
	override fun onViewBindingCreated(binding: DialogDownloadBinding, savedInstanceState: Bundle?) {
		...
		optionViews = arrayOf(
			...
			binding.optionNotDownloadedChapters,
		).onEach {
	...
	override fun onClick(v: View) {
		when (v.id) {
			...
			R.id.button -> when (v.parentView?.id ?: return) {
				...
				R.id.option_not_downloaded_chapters -> showNotDownloadedChaptersCountSelection(v)
			}
		...
		viewBinding?.run {
			...
			viewModel.confirm(
				...
				chaptersMacro = when {
					...
					optionNotDownloadedChapters.isChecked -> options.notDownloadedChapters ?: return@run
					else -> return@run
				},
	...
	private fun onChapterSelectOptionsChanged(options: ChapterSelectOptions) {
		with(viewBinding ?: return) {
			...
			// Next N chapters after the latest downloaded chapter
			optionNotDownloadedChapters.isVisible = options.notDownloadedChapters != null
			options.notDownloadedChapters?.let {
				optionNotDownloadedChapters.title = if (it.chaptersCount == Int.MAX_VALUE) {
					resources.getString(R.string.download_option_all_after_latest_download)
				} else {
					resources.getString(
						R.string.download_option_next_not_downloaded_n_chapters,
						resources.getQuantityStringSafe(
							R.plurals.chapters,
							it.chaptersCount,
							it.chaptersCount,
	...
	private fun showNotDownloadedChaptersCountSelection(v: View) {
		val option = viewModel.chaptersSelectOptions.value.notDownloadedChapters ?: return
		val menu = PopupMenu(v.context, v)
		chaptersCount(option.maxAvailableCount).forEach { i ->
			menu.menu.add(i.format())
		}
		menu.menu.add(getString(R.string.chapters_all))
		menu.setOnMenuItemClickListener {
			viewModel.setNotDownloadedChaptersCount(
				it.title?.toString()?.toIntOrNull() ?: Int.MAX_VALUE,
			)
			true
		}
		menu.show()
